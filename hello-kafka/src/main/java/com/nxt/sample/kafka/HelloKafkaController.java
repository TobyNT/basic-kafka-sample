...
public class HelloKafkaController {
	...
	public void listenAsString(ConsumerRecord<String, String> crecord, @Payload String payload) {
		logger.info("Logger 2 [String] received key {}: Type[{}] | Payload: {} | Record: {}", crecord.key(),
				typeIdHeader(crecord.headers()), payload, crecord.toString());
		latch.countDown();
	}

	@KafkaListener(topics = "advice-topic", clientIdPrefix = "bytearray", containerFactory = "kafkaListenerByteArrayContainerFactory")
	public void listenAsString(ConsumerRecord<String, byte[]> crecord, @Payload byte[] payload) {
		logger.info("Logger 3 [ByteArray] received key {}: Type[{}] | Payload: {} | Record: {}", crecord.key(),
				typeIdHeader(crecord.headers()), payload, crecord.toString());
		latch.countDown();
	}
